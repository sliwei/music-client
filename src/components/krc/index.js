...
export default class Index extends React.Component {
  ...
  render() {
    ...
    return <div className={css.now_music}>
      <div style={{width: `${krcSta ? '100%' : '0'}`, transform: `scale(${krcSta ? 1 : 0})`}} className={css.krc}>
        ...
        <div className={css.krc_box}>
          <div className={css.krc_box_info}>
            <img style={{animation: !paused ? 'rotating 20s linear infinite' : ''}}
                 src={'imgUrl' in nowMusic ? nowMusic.imgUrl.replace('{size}', '400') : null} alt=""/>
      ...
      <img onClick={this.changeKrc} className={css.now_music_img}
           src={'imgUrl' in nowMusic ? nowMusic.imgUrl.replace('{size}', '400') : null} alt=""/>
