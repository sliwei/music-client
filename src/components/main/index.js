...
export default class Index extends React.Component {
  ...
  getData = () => {
    op({url: 'http://m.kugou.com/?json=true'}).then(res => {
      console.log(res);
      this.setState({banner: res.data.banner, songs: res.data.data})
    })
  };
        ...
        {banner.map((item, i) => {
          return <Element prefixCls="banner-user-elem" key={i}>
            <BgElement key={i} style={{textAlign: 'center'}}>
              <Link key={i} to={'specialid' in item.extra ? `/song_list/${item.extra.specialid}` : ''}
                    className={css.he_ZoomInImg_caption_a}>
